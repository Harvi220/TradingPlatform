        ...
        <div className="flex justify-between items-start mb-2">
          <div>
            ...
            <div className="text-xs text-orange-600 font-semibold mt-1">
              ⚠️ Данные обновляются раз в минуту. Для актуальных данных смотрите таблицу WebSocket выше.
            </div>
          </div>
          {/* Индикатор обновления и кнопка */}
          <div className="flex flex-col items-end gap-2">
            ...
            <button
              ...
              disabled={loading}
              className="px-3 py-1 text-xs font-medium text-white bg-purple-600 hover:bg-purple-700 disabled:bg-gray-400 rounded transition-colors"
            >
          ...
          <table className="min-w-full bg-white border border-gray-300 text-sm">
            ...
            <tbody>
              {data.depthDetails.map((detail, index) => {
                ...
                // 2. ХОТЯ БЫ ОДНО количество ордеров (BID ИЛИ ASK) точно одинаковое с предыдущей глубиной
                // const prevDetail = index > 0 ? data.depthDetails[index - 1] : null;

                // Проверяем одинаковые значения
                // const isBidCountSame = prevDetail && detail.bidOrderCount === prevDetail.bidOrderCount;
                // const isAskCountSame = prevDetail && detail.askOrderCount === prevDetail.askOrderCount;

                // const hasNoData =
                //   (detail.bidOrderCount === 0 && detail.askOrderCount === 0) ||
                //   isBidCountSame ||  // Количество BID ордеров одинаковое
                //   isAskCountSame;    // Количество ASK ордеров одинаковое
                ...
                return (
                  <tr key={detail.depth} className="hover:bg-gray-50">
                    ...
                    {/* ЗАКОММЕНТИРОВАНО: Скрытие столбцов при недостаточных данных */}
                    {/* {hasNoData ? (
                      <td className="px-2 py-2 border text-center text-gray-400 italic" colSpan={6}>
                        Недостаточно данных на этой глубине
                      </td>
                    ) : ( */}
                      <>
